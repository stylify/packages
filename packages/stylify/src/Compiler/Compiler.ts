...
export default class Compiler {
	...
	public addComponent(selector: string, selectorDependencies: string[] | string): Compiler {
		...
		selectorDependencies.forEach((selectorDependency) => {
			...
			if (this.componentsSelectorsMap[selectorDependency].selectors.indexOf(selector) === -1) {
				this.componentsSelectorsMap[selectorDependency].selectors.push(selector);
			}
		});
 */
	...
	public compile(content: string, compilationResult: CompilationResult = null): CompilationResult {
		...
		const notProcessedComponentsSelectors = Object.keys(this.components).filter((element): bool => {
		...
		if (notProcessedComponentsSelectors.length) {
			...
			for (const notProcessedComponentsSelector of notProcessedComponentsSelectors) {
				const componentSelectorRegExp = new RegExp('(?:^|)(' + notProcessedComponentsSelector + ')(?:$| )', 'igm');
	...
	public hydrate(data: Record<string, any>): void {
		Object.keys(this.components).forEach(selector => {
			for (const componentSelector of this.components[selector].selectors) {
				if (! (componentSelector in data.selectorsList && data.selectorsList[componentSelector].processed === true)) {
					continue;
				}
	...
	private processMacros(content: string, compilationResult: CompilationResult = null) {
		...
		for (const macroKey in this.macros) {
			...
			while (macroMatches = macroRe.exec(content)) {
				...
				const macroIsProcessed = macroIsInSelectorsList && compilationResult.selectorsList[fullMatch].processed === true;
